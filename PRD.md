...
- 點數系統：
  ...
  - 問答失敗（系統錯誤/超時）自動回補點數。
- 金流與購點：固定點數包購買與交易紀錄。
- 延伸問題（可選）：系統可提供延伸問題，使用者可先保存，啟用時再扣點。
...
## 11. 待確認的決策點
- 每次提問扣點規則：固定扣點或依模式/長度動態計費？
- 點數包方案與售價（例如 100/300/1000 點）。
- 是否強制 Email 驗證後才能提問。
- 是否在前台顯示「參考來源摘要」（不露出內部規則名）。
- 首版是否要開啟「延伸問題保存與下次啟用」功能。
- 規則型問題（deterministic）與一般 RAG 的判斷條件由誰維護（產品/營運/工程）。
